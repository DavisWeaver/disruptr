...
compute_null <- function(cache = NULL, df, ppi = "biogrid", n,
  ...
  combine_null <- function(x, y, ...) {

    #bind together all the arguments that aren't the aggregate
    args <- lapply(list(y, ...), function(z) z[[2]])

    #don't want to try and compute sd with one sample... thats where the trouble is ocming I t
    if(y[[1]] == 2) {
      x[[1]] <- dplyr::bind_rows(x[[2]], args) %>%
        dplyr::group_by(.data$gene_name, .data$sample_name) %>%
        dplyr::summarise(mean_dnp = mean(.data$dnp),
                         sd_dnp = sd(.data$dnp, na.rm = TRUE),
                         n = dplyr::n())
      x[[2]] <- list() #need to clear out x[[2]] any time we add to x1
    } else {
      x[[2]] <- c(x[[2]], args)
    }

    if(length(x[[2]]) >= 10) { #condition where x is pre-combined version
      x_agg = x[[1]]
      #bind everything together
      df <- dplyr::bind_rows(x[[2]])
      x[[2]] <- list() #clear out the "memory" any time we re-compute x[[1]]
      df <- df %>%
        dplyr::group_by(.data$gene_name, .data$sample_name) %>%
        dplyr::summarise(mean_dnp = mean(.data$dnp),
                         sd_dnp = sd(.data$dnp, na.rm = TRUE),
      ...
      #re-order df to be in the same order as x
      new_index = match(df$gene_name, x_agg$gene_name)
      df <- df[order(new_index),]

      #compute weighted averages of everything
      mean_dnp <- vector(mode = "numeric", length = nrow(df))
      sd_dnp <- vector(mode = "numeric", length = nrow(df))
      n_vec <- vector(mode= "integer", length = nrow(df))
      for(i in 1:length(mean_dnp)) {
        mean_dnp[i] <-
          (x_agg$mean_dnp[i]*x_agg$n[i] + df$mean_dnp[i]*df$n[i]) /
          (x_agg$n[i] + df$n[i])
        sd_dnp[i] <-
          (x_agg$sd_dnp[i]*x_agg$n[i] + df$sd_dnp[i]*df$n[i]) /
          (x_agg$n[i] + df$n[i])
        n_vec[i] <- x_agg$n[i] + df$n[i]
      }

      x[[1]] <- data.frame(gene_name = df$gene_name, sample_name = df$sample_name,
                           mean_dnp = mean_dnp, sd_dnp = sd_dnp, n = n_vec)
  ...
  final_combine <- function(x) {
    ...
    x <- data.frame(gene_name = df$gene_name, sample_name = df$sample_name,
                    mean_dnp = mean_dnp, sd_dnp = sd_dnp, n = n_vec)
    return(x)
  }
